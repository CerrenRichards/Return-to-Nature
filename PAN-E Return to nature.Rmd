...
## Summarise the continental differences
...
# Exploring countries with <10 days difference

```{r}
# Join the lockdown dates to the full Google data for plotting
returnplots <- left_join(google_countries, returns, by = c("country", "ISO3"))

returnplots %>% filter(necessities_diff <= 20)
...
regions <- returns %>% group_by(continent) %>% 
            summarise(parks = median(naturediff, na.rm = T),
                      parks.min = min(naturediff, na.rm = T),
                      parks.max = max(naturediff, na.rm = T),
                      lux = median(luxdiff, na.rm = T),
...
## Set the map theme
maptheme <-  theme_void(base_size = 15)+
  theme(axis.title.y = element_blank(), 
        axis.title.x = element_blank(),
...
map.regions <- regions %>% 
            select(subregion, countries, parks, lux, long, lat)  %>%
...
ggplot(data = map.regions,
    ...
    scale_fill_manual(values = c("#00868B", "#FF6A00"))+
 ...
 library(ggsubplot)
 map.plot+ 
   geom_subplot2d(aes(long, lat, subplot = geom_bar(aes(x = Mobility, y = Difference, fill = Mobility, width=1), 
                                                    position = "identity")), ref = NULL, data = map.regions)
    ...
    coord_sf(xlim = c(-110, -25), ylim = c(35, 80), expand = FALSE)+
    maptheme+

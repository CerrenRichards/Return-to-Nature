...
# Filter for only 2021 data to find the countries that don't pass zero in 2020
google$date2 <- as.Date(google$date)
google <- google %>% filter(date2 <= "2021-12-31")
...
## We have two years of data so we will need to not mix up the julian dates 
## When plotting with a date in ggplot, it can sometimes take a long time, so here we
## Will use id
google_countries<- google_countries %>% group_by(ISO3) %>% mutate(id = row_number())
...
## exploring the luxuries longer
```{r}
ggplot(data = google2, aes(x=date, y=`Retail & Recreation`)) + 
  geom_line() + 
  facet_wrap_paginate(~country, ncol = 7, nrow = 7, scale="free_y", page = 1)
```

## Date countries cross zero (2021 too)
```{r}
library(tidyverse)
...
# Nature (Parks and Beaches)
nature <- google_countries %>% group_by(country, ISO3) %>% 
            filter(julian > max.home & `Parks & Beaches` > 0) %>%
            summarise(home =  min(max.home),
                      parks_zero = min(julian),
                      parks_diff = parks_zero - home)

# Necessities (Groceries and pharmacies)
necessities <- google_countries %>% group_by(country, ISO3) %>% 
            filter(julian > max.home & `Grocery & Pharmacy` > 0) %>%
            summarise(home =  min(max.home),
                      necessities_zero = min(julian),
                      necessities_diff = necessities_zero - home)

# Luxuries (Retail & Recreation)
luxuries <- google_countries %>% group_by(country, ISO3) %>% 
            filter(julian > max.home & `Retail & Recreation` > 0) %>%
            summarise(home =  min(max.home),
                      luxuries_zero = min(julian),
                      luxuries_diff = luxuries_zero - home)
...
median(returns$naturediff, na.rm = T) # 20 days after necessities
median(returns$luxdiff, na.rm = T) # 63 after necessities
...
## NESSESITIES
ggplot(data = returnplots, aes(x=id, y=`Grocery & Pharmacy`)) + 
  geom_line() + 
  facet_wrap_paginate(~country, ncol = 7, nrow = 7, scale="free_y", page = 1)+

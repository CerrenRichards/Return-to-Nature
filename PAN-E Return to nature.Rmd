...
## Explore confinement differences

The intensity of government measures in the period between peak lockdown and the pulse back to nature for each country
...
nature.conf <- googleconf %>% group_by(country, ISO3) %>% 
            ...
            summarise(home =  min(max.home), # the date people were home most
                      ...
                      stringency = median(StringencyIndex_Average),
...
nature.conf2 <- left_join(nature.conf, returns)



ggplot(nature.conf)+
  geom_boxplot(aes(subregion, gov.index))
...
ggplot(filter(google_countries, country == "Canada"))+
  geom_line(aes(Date, Residential))
...
for (i in unique(google_countries$country)) {
  ...
  # Fit the GAM models 
  # k = 30 for the number of months in the dataset
  gam_lux.pred <- gam(Luxury ~ s(time, k = 20), data = country_data)
  gam_nat.pred <- gam(Nature ~ s(time, k = 20), data = country_data)
  gam_nec.pred <- gam(Necessities ~ s(time, k = 20), data = country_data)
  gam_home.pred <- gam(Residential ~ s(time, k = 20), data = country_data)
  ...
  predict_nat[[i]] <- predict.gam(gam_nat.pred, type="response")
  predict_nec[[i]] <- predict.gam(gam_nec.pred, type="response")
  predict_home[[i]] <- predict.gam(gam_home.pred, type="response")
  
  # DETRENDED DATA
  # Fit the GAM models 
  gam_lux.pred.resid <- gam(center.Lux ~ s(time, k = 20), data = country_data)
  gam_nat.pred.resid <- gam(center.Nat ~ s(time, k = 20), data = country_data)
  gam_nec.pred.resid <- gam(center.Nec ~ s(time, k = 20), data = country_data)
...
# home
google_countries2 <- google_countries %>% drop_na(Residential)
google_countries2$home.pred <- unlist(lapply(countries, function(country) predict_home[[country]]))
google_countries <- left_join(google_countries, google_countries2)
...
## Make plot for conceptual diagram
```{r}


can <- filter(google_countries, country == "Canada") %>% slice(1:320)
write.csv(can, "conceptual.csv")


conceptual <- read.csv("conceptual.csv")
conceptual$Date <- as.POSIXct(conceptual$Date, format='%Y-%m-%d')

# Plot Nature
ggplot(conceptual) +
  theme_bw(base_size = 15)+
  geom_hline(yintercept = 0, alpha = 0.4)+
    geom_line(aes(Date, Nature), colour = "#00868B", size = 1.1)+
  geom_line(aes(Date, Residential), size = 1.1)+
    geom_line(aes(Date,Detrended), colour = "Light green", size = 1.1)+
  #geom_vline(linetype = "dashed", xintercept = as.POSIXct("2020-04-19"))+ # peak lockdown
  #geom_vline(linetype = "dashed",colour = "#00868B", xintercept = as.POSIXct("2020-05-20"))+ # return nature
 #geom_vline(linetype = "dashed",colour = "Light green", xintercept = as.POSIXct("2020-06-20"))+ # return detrended
  theme(panel.grid.major = element_blank(), # remove the major lines
        panel.grid.minor = element_blank())+
  labs(y = "Change in Mobility (%)")

# Save the plot
ggsave("conceptual diagram.png", 
...
ggplot(can) +
  theme_bw()+
  geom_line(aes(Date, Nature), alpha = 0.2)+
   geom_line(aes(Date, Residential), colour = "green", alpha = 0.4)+
  geom_line(aes(Date, res.Nature), colour = "blue", alpha = 0.4)+
   #geom_point(aes(Date,center.Nat),colour = "blue",  alpha = 0.2)+
    geom_line(aes(Date, Nature.pred), colour = "black", size = 1.25)+
  geom_line(aes(Date, Nature.pred.resid), colour = "blue", size = 1.25)+
    geom_line(aes(Date, home.pred), colour = "green", size = 1.25)

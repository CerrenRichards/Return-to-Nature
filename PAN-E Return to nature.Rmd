...
# Set colors
world <- mutate(world, fill = ifelse(ISO3 %in% returns$ISO3, "Monitored", "Non-monitored"))
...
ggplot() +
    ...
    #           aes(x = long, y = lat, label = subregion), box.padding = 0.5, max.overlaps = Inf) +
...
map.regions2 <- regions %>% 
            select(subregion, countries, nature, luxuries, necessities, long, lat)  %>%

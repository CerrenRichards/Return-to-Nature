...
library(mgcv)
m.lux<-gam(Luxury~ s(Month), random=list(country_region=~1), data=google_countries)
google_countries$res.Luxury<-residuals(m.lux)

res.Luxury <- data.frame(Luxury = m.lux$model$Luxury, Month = m.lux$model$Month, res.lux = m.lux$residuals)

google_countries2 <- left_join(res.Luxury, google_countries)

google_countries2 <- google_countries %>% drop_na(Luxury)
google_countries2$res.Luxury<-residuals(m.lux)

google_countries3 <- left_join(google_countries, google_countries2)
...
plot(data=can,Luxury~X)
m1<-gam(Luxury~s(Month),data=can)
can$res.Luxury<-residuals(m1)
plot(data=can,res.Luxury~X)



m1<-gamm(Luxury~s(Month), random=list(country_region=~1), data=can)

## like the burrow code:
bs = re

...
# ****When running the GAMM - need to add the extra $gam
library(mgcv)
...
## ****** Stopped Here - update - apply across all countries

# Recenter the residuals based on the difference between the means
google_countries$recenter.lux <- google_countries$res.Luxury - 13.6430597
...
# Model
m2<-gam(Nature~s(Month),data=can)

# Calculate residuals
can$res.Nature<-residuals(m2)


# Calculate the mean of Luxury and the model residuals for the first two weeks
# Then calculate the difference between the means 
can %>% select(Nature, res.Nature) %>% 
  slice(1:14) %>% summarise(mean.nat = mean(Nature),
                            mean.res = mean(res.Nature),
                            diff = mean.res - mean.nat)


# Recenter the residuals based on the difference between the means
can$recenter.nat <- can$res.Nat - - 4.548827
...
ggplot(can) +
  ...
  geom_point(aes(Date, Nature))+
   ...
   geom_point(aes(Date,recenter.nat),colour = "blue",  alpha = 0.4)
...
# ****When running the GAMM - need to add the extra $gam
library(mgcv)
m.nat<-gamm(Nature~ s(Month), random=list(country=~1), data=google_countries)

# Explore model as GAM
m.nat2<-gam(Nature~ s(Month) + s(country, bs="re"), method = "REML", data=google_countries)
...
google_countries2$res.Nature<-residuals(m.nat2)
google_countries <- left_join(google_countries, google_countries2)


# Calculate the mean of Luxury and the model residuals for the first two weeks
# Then calculate the difference between the means 
recenter<- google_countries %>% group_by(country) %>%  select(Nature, res.Nature) %>% 
  slice(1:14) %>% summarise(mean.Nat = mean(Nature),
                            mean.res = mean(res.Nature),
                            diff = mean.res - mean.Nat)
...
# Recenter the residuals based on the difference between the means
google_countries$recenter.nat <- google_countries$res.Nature - -50.2580508
...
regions <- returns %>% group_by(subregion) %>% 
            summarise(nature = median(parks_diff, na.rm = T),
...
## Add a number column to subsitute for date in the model
google_median$time <- 1:nrow(google_median)
...
## Test whether duration of time in necessities, nature and luxuries is statistically different over time

```{r}
## Change to factor for the model to run
google_median$type <- as.factor(google_median$type)


google_median$Month <- format(google_median$Date,"%m")
google_median$Month <- as.numeric(google_median$Month)


m.bam<- bam(data = google_median, change~s(time) + s(type, bs="re"), method = 'REML')
acf(resid(m.bam))
...
m.test<- gam(data = google_median, change~s(scale(time)) + s(type, bs="re"), method = 'REML')

m.test2<- gam(data = google_median, 
             change~s(time, bs="ts", k = 40) +
               s(Month, bs = "cc")+
               s(type, bs="re"), method = 'REML')
...
m.test2<- gamm(data = google_median, 
              change~s(time),
              random = list(type = ~1), 
              corr = corAR1(form = ~ 1|type),
              method = 'REML')

summary(m.test2$lme)
plot(m.test2$gam)
hist(resid(m.test2$gam))
acf(resid(m.test2$lme, type = "normalized"))
gam.check(m.test2$gam)
...
# in our case.
nBurrow <- length(unique(extreme_cold_ATPU$Burrow))
ar.strt <- rep(c(TRUE, rep(FALSE, (nrow(extreme_cold_ATPU)/nBurrow) - 1)), 
               length(unique(extreme_cold_ATPU$Burrow)))
...
for(i in 1:length(rho)) {
  ...
  bam1_buffer <- bam(
    ...
    rho = rho[i],
    AR.start = ar.strt,
    data = extreme_cold_ATPU,
    method = "REML"
  )
  REML[i] <- bam1_buffer$gcv.ubre # Save the REML score
}
...
acf(bam2_buffer$std.rsd)
summary(c)
AIC(gam1, gam3, gam4)
summary(gam4)
plot(bam2_buffer)
gam.check(bam2_buffer)
acf(residuals(gam4))
gam.check(bam2_buffer, pages = 1)
plot(bam2_buffer, scheme = 2, pages = 1)
```

...
## Run GAM models through the data to describe the trend
...
for (i in unique(google_countries$country)) {
  ...
  # Extract gam trend and store in the lists
...
ggplot(google_countries) +
  ...
  geom_point(aes(Date, Luxury))+
   #geom_point(aes(Date,res.Luxury),colour = "red", alpha = 0.4)+
   geom_point(aes(Date,center.Lux),colour = "blue",  alpha = 0.4)+
  geom_line(aes(Date, Luxury.pred), colour = "red")+
  geom_line(aes(Date, Luxury.pred.resid), colour = "yellow")+
  facet_wrap(~country, scales = "free_y")
...
ggplot(filter(google_countries, country %in% c("United Kingdom", "Canada", "Norway"))) +
  ...
  geom_point(aes(Date, Nature))+
   #geom_point(aes(Date,res.Nature),colour = "red", alpha = 0.4)+
   geom_point(aes(Date,center.Nat),colour = "blue",  alpha = 0.4)+
    geom_line(aes(Date, Nature.pred), colour = "red", size = 1.25)+
  geom_line(aes(Date, Nature.pred.resid), colour = "yellow", size = 1.25)+
  facet_wrap(~country, scales = "free_y", ncol = 1)
...
ggplot(google_countries) +
  ...
  # geom_point(aes(Date,res.Necessities),colour = "red", alpha = 0.4)+
   geom_point(aes(Date,center.Nec),colour = "blue",  alpha = 0.4)+
    geom_line(aes(Date, Necessities.pred), colour = "red", size = 1.25)+
  geom_line(aes(Date, Necessities.pred.resid), colour = "yellow", size = 1.25)+
  facet_wrap(~country, scales = "free_y")
...
## UPDATED Date countries cross zero
...
nature.resid <- google_countries %>% group_by(country, ISO3) %>% 
            ...
            summarise(home =  min(max.home), # the date people were home most
                      parks_zero.resid = min(Date), # The first date people returned to nature
                      parks_diff.resid = parks_zero.resid - home) # calc difference 
...
nature.gam <- google_countries %>% group_by(country, ISO3) %>% 
            ...
            summarise(home =  min(max.home), # the date people were home most
                      parks_zero.gam = min(Date), # The first date people returned to nature
                      parks_diff.gam = parks_zero.gam - home) # calc difference 

# Nature raw resid
nature.resid.gam <- google_countries %>% group_by(country, ISO3) %>% 
            filter(Date > max.home & Nature.pred.resid > 0) %>%
            summarise(home =  min(max.home), # the date people were home most
                      parks_zero.resid.gam = min(Date), # The first date people returned to nature
                      parks_diff.resid.gam = parks_zero.resid.gam - home) # calc difference 

            

## Join all the nature dataframes
nature <- left_join(nature, nature.resid)%>%
...
# NATURE
median(nature$parks_diff, na.rm = T) # 78 after peak lockdown
median(nature$parks_diff.resid, na.rm = T) # 129 days
median(nature$parks_diff.gam, na.rm = T) # 93 days
median(nature$parks_diff.resid.gam, na.rm = T) # 319 days

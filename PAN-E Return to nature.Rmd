...
home <- google_countries %>% group_by(country, ISO3) %>% 
            filter(julian <= 122) %>%
            filter(Residential == max(Residential)) %>%
...
mean(returns$naturediff, na.rm = T) #20 days after nessesities
mean(returns$luxdiff, na.rm = T) #63 after nessesities
...
## NESSESITIES
ggplot(data = returnplots, aes(x=julian, y=`Grocery & Pharmacy`)) + 
  geom_line() + 
  facet_wrap_paginate(~country, ncol = 7, nrow = 7, scale="free_y", page = 1)+
...
## NATURE
ggplot(data = returnplots, aes(x=julian, y=`Parks & Beaches`)) + 
  geom_line() + 
  facet_wrap_paginate(~country, ncol = 7, nrow = 7, scale="free_y", page = 1)+
  geom_hline(aes(yintercept = 0), linetype = "dashed")+
  geom_vline(aes(xintercept = max.home), colour = "red")+

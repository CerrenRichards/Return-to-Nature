...
test <- google_countries %>% filter(country %in% c("Canada", "Costa Rica", "Zimbabwe", "Australia"))
...
for (i in unique(test$country)) {
  
  # Subset the data for the current country
  country_data <- subset(test, country == i)
  
  # Fit the GAM model
  gam_model <- gam(Luxury ~ s(Month), data = country_data)
  
  # Extract residuals and store in the list
  residuals_list[[i]] <- residuals(gam_model)
}


# Add residuals back to the original dataframe
test$res.Luxury <- unlist(lapply(countries, function(country) residuals_list[[country]]))
...
# Luxury
test2 <- test %>% drop_na(Luxury)
test2$res.Luxury <- unlist(lapply(countries, function(country) residuals_lux[[country]]))
test <- left_join(test, test2)

# Nature
test2 <- test %>% drop_na(Nature)
test2$res.Nature<-unlist(lapply(countries, function(country) residuals_nat[[country]]))
test <- left_join(test, test2)

# Necessities
test2 <- test %>% drop_na(Necessities)
test2$res.Necessities <- unlist(lapply(countries, function(country) residuals_nec[[country]]))
test <- left_join(test, test2)
...
recenter<- test %>% group_by(country) %>%  select(Luxury, res.Luxury) %>% 
  slice(1:25) %>% summarise(mean.Lux = mean(Luxury),
                            mean.res = mean(res.Luxury),
                            diff.Lux = mean.res - mean.Lux)%>%
                  select(country, diff.Lux)


# Join datasets
test <- left_join(test, recenter)

# Recenter the residuals based on the difference between the means
test$center.Lux <- test$res.Luxury - test$diff.Lux
